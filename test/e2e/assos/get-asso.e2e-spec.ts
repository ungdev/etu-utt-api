...
const GetAssoE2ESpec = e2eSuite('GET /assos/:id', (app) => {
  ...
  it('should return a 400 as the id param is not valid', () =>
    pactum.spec().get('/assos/thisisnotavaliduuid').expectAppError(ERROR_CODE.PARAM_NOT_UUID, 'assoId'));

  it('should return a 404 as asso is not found', () =>
    pactum.spec().get(`/assos/${Dummies.UUID}`).expectAppError(ERROR_CODE.NO_SUCH_ASSO, Dummies.UUID));
