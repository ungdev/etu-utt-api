...
const GetMyUesE2ESpec = e2eSuite("GET ue/of/me", (app) => {
  ...
  const semester = createSemester(app, {start: new Date(Date.now() - 30 * 24 * 3_600_000), end: new Date(Date.now() + 30 * 24 * 3_600_000)});
  createUESubscription(app, {user, ue, semester});
  const ue2 = createUE(app);
  const semester2 = createSemester(app, {start: new Date(Date.now() - 90 * 24 * 3_600_000), end: new Date(Date.now() - 30 * 24 * 3_600_000)});
  ...
  it('should fail as the user is not authenticated', () => pactum.spec().get('/ue/of/me').expectAppError(ERROR_CODE.NOT_LOGGED_IN));

  it('should fail as user is not a student', () => pactum.spec().get('/ue/of/me').withBearerToken(formerStudent.token).expectAppError(ERROR_CODE.FORBIDDEN_INVALID_ROLE, 'STUDENT'));
