...
export class UEService {
      ...
      ...(query.q
        ? {
            OR: [
              ...
              {
                info: {
                  OR: [{ comment: query.q }, { objectives: query.q }, { programme: query.q }],
                },
              },
            ],
          }
  ...
  async getComments(
    ...
    for (const comment of comments)
      if (comment.isAnonymous && !bypassAnonymousData && comment.author.id !== user.id) delete comment.author;
    return {
  ...
  async replyComment(user: FakeUser, commentId: string, reply: CommentReplyDto): Promise<UECommentReply> {
