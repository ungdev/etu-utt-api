...
export class AuthController {
  ...
  @ApiAppErrorResponse(ERROR_CODE.CREDENTIALS_ALREADY_TAKEN, 'Login, email address or any field that should be unique is already taken')
  ...
  @ApiAppErrorResponse(ERROR_CODE.INVALID_TOKEN_FORMAT, 'The token is not in the right format. This may happen if you did not use the syntax `Bearer {token there}`, or the token was not generated by the API')
  ...
  @ApiCreatedResponse({description: 'The account was successfully created. The token to authenticate the following requests is passed in the body'})
  @ApiAppErrorResponse(ERROR_CODE.INVALID_TOKEN_FORMAT, 'The format of the token is invalid. This may happen if the token used is not one provided by the API')
  @ApiAppErrorResponse(ERROR_CODE.CREDENTIALS_ALREADY_TAKEN, 'Login, email, or any other field that should be unique about a user is already bound to another user')
