...
export const ApiAppErrorResponse = (error: ERROR_CODE, description: string = "") => {
  ...
  // @ts-ignore
  const ResponseDtoMixin = mixin(ResponseDto, `AppErrorResponseDto$${error}`);
...
export function paginatedResponseDto<TBase extends Constructor>(
  ...
  return mixin(ResponseDto, `PaginatedResponseDto$${Base.name}`); // This is important otherwise you will get always the same instance
}
